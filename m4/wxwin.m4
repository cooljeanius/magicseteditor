...
dnl#
dnl# The "WX_DEBUG_CONFIGURE" variable can be set to 1 to enable extra
...
dnl#                but wx-config script could NOT be found.
dnl#
dnl#                Please check that wx-config is in path, the directory
dnl#                where wxWidgets libraries are installed (returned by
dnl#                'wx-config --libs' command) is in LD_LIBRARY_PATH or
dnl#                equivalent variable and wxWidgets version is 2.3.4 or
dnl#                above.
dnl#        ])
dnl#     fi
...
dnl#
dnl# If you want to support standard --enable-debug/unicode/shared options,
dnl# you may do the following:
dnl#
dnl#     ...
...
[
    AC_ARG_WITH([wxdir],
                [AS_HELP_STRING([--with-wxdir=PATH],[Use uninstalled version of wxWidgets in PATH])],
                [wx_config_name="$withval/wx-config"
                 wx_config_args="--inplace"])
    AC_ARG_WITH([wx-config],
                [AS_HELP_STRING([--with-wx-config=CONFIG],[wx-config script to use (optional)])],
                [wx_config_name="$withval"])
    AC_ARG_WITH([wx-prefix],
                [AS_HELP_STRING([--with-wx-prefix=PREFIX],[Prefix where wxWidgets is installed (optional)])],
                [wx_config_prefix="$withval"],[wx_config_prefix=""])
    AC_ARG_WITH([wx-exec-prefix],
                [AS_HELP_STRING([--with-wx-exec-prefix=PREFIX]
                   [Exec prefix where wxWidgets is installed (optional)])],
                [wx_config_exec_prefix="$withval"],[wx_config_exec_prefix=""])
])

dnl# Helper macro for checking if wx version is at least $1.$2.$3, sets
...
dnl# (the latter is for static linking with wxWidgets). Set WX_CONFIG_NAME
dnl# environment variable to override the default name of the wx-config
dnl# script to use. Set WX_CONFIG_PATH to specify the full path to
dnl# wx-config - in this case the macro will NOT even waste time on tests
dnl# for its existence.
dnl#
dnl# Optional WX-LIBS argument contains comma- or space-separated list of
dnl# wxWidgets libraries to link against. If it is not specified,
dnl# then WX_LIBS and WX_LIBS_STATIC will contain flags to link with all
dnl# of the core wxWidgets libraries.
dnl#
...
[
  ...
  dnl# do NOT search the PATH if WX_CONFIG_NAME is absolute filename
  ...
  if test "$WX_CONFIG_PATH" != "no" ; then
    ...
    dnl# do NOT add libraries ($4) to this variable as this would result in
    dnl# an error when it is used with --version below
  ...
  dnl# to support wxpresets bakefiles (we export also WX_VERSION_MICRO for
  dnl# completeness):
...
dnl#                where wxWidgets libraries are installed (returned by
dnl#                'wx-config --libs' command) is in LD_LIBRARY_PATH or
dnl#                equivalent variable and wxWidgets version is 2.6.0 or
dnl#                above.
dnl#        ])
dnl#     fi
dnl#
...
dnl#
dnl# Test for wxWidgets' wxrc program for creating C++, Python or XRS
dnl# resources. The variable WXRC will be set and substituted in the
dnl# configure script and Makefiles.
dnl#
dnl# Example use:
...
AC_DEFUN([WXRC_CHECK],
[
  AC_ARG_VAR([WXRC],[Path to wxWidgets' wxrc resource compiler])
...
dnl#
dnl# Sets the "output-var" variable to the name of a library named
dnl# with same wxWidgets rule.
...
dnl#
dnl# Two little custom macros which define the ENABLE/WITH configure
dnl# arguments.
dnl# Macro arguments:
...
AC_DEFUN([WX_ARG_WITH_YESNOAUTO],
         [AC_ARG_WITH([$1],
            ...
            if test "$withval" = "yes" ; then
                ...
                $2=1
                $5
            dnl# NB: by default we do NOT allow --with-$1=no option
...
dnl#   --with-wxversion
dnl# Then checks for their presence and eventually set the DEBUG, UNICODE,
dnl# SHARED, PORT, WX_SHARED, WX_DEBUG, variables to one of the "yes",
dnl# "no", "auto" values.
dnl#
...
dnl# if wxWidgets was built in Unicode mode (and still is in boolean
dnl# format).
dnl#-----------------------------------------------------------------------
AC_DEFUN([WX_STANDARD_OPTIONS],
        [

        dnl# the following lines will expand to WX_ARG_ENABLE_YESNOAUTO
        dnl# calls if and only if the $1 argument contains respectively
        dnl# the debug,unicode or shared options.

        dnl# be careful here not to set debug flag if only "wxdebug" was
        dnl# specified
        ...
        dnl# WX_ARG_WITH_YESNOAUTO cannot be used for --with-toolkit since
        dnl# it is an option which must be able to accept the
        dnl# auto|gtk1|gtk2|msw|... values
    ...
    Unrecognized option value (allowed values: auto, gtk1, gtk2, msw, motif, osx_carbon, osx_cocoa, dfb, x11)
        ...
        dnl# shared mode against a static build of wxWidgets. Thus we have
        dnl# the following option which allows these mixtures. E.g.
        dnl#
        dnl#     ./configure --disable-shared --with-wxshared
        dnl#
        dnl# will build your library in static mode against the 1st
        dnl# available shared build of wxWidgets.
        dnl#
        dnl# Note that it is not possible to do the viceversa:
        dnl#
        dnl#      ./configure --enable-shared --without-wxshared
        dnl#
        dnl# Doing so you would try to build your library in shared mode
        dnl# against a static build of wxWidgets. This is not possible
        dnl# (you would mix PIC and non PIC code)!
        dnl# A check for this combination of options is in
        dnl# WX_DETECT_STANDARD_OPTION_VALUES (where we know what 'auto'
        dnl# should be expanded to).
        dnl#
        dnl# If you try to build something in ANSI mode against a UNICODE
        dnl# build of wxWidgets or in RELEASE mode against a DEBUG build
        dnl# of wxWidgets, then at best you will get ton of linking errors!
        dnl# ************************
        ...
        dnl# Just like for SHARED and WX_SHARED it may happen that some
        dnl# adventurous peoples will want to mix a wxWidgets release build
        dnl# with a debug build of his app/lib. So, we have both DEBUG and
        dnl# WX_DEBUG variables.
        ...
        dnl# WX_ARG_WITH_YESNOAUTO cannot be used for --with-wxversion
        dnl# since it is an option which accepts the
        dnl# "auto|2.6|2.7|2.8|2.9|3.0" etc etc values
...
dnl# WX_CONVERT_STANDARD_OPTIONS_TO_WXCONFIG_FLAGS
dnl#
dnl# Sets WXCONFIG_FLAGS string using SHARED,DEBUG,UNICODE variable values
dnl# which are different from "auto".
dnl# Thus this macro needs to be called only once all options are set.
dnl#-----------------------------------------------------------------------
...
AC_DEFUN([_WX_SELECTEDCONFIG_CHECKFOR],
        ...
        if test "$$1" = "auto" ; then
            ...
            dnl# The user does not have particular preferences for this
            dnl# option; so we will detect the wxWidgets relative build
            dnl# setting and use it
            AC_MSG_CHECKING([$3])

            dnl# set WX_$1 variable to 1 if the $WX_SELECTEDCONFIG
            dnl# contains the $2 string or to 0 otherwise.
            dnl# NOTE: 'expr match STRING REGEXP' cannot be used since on
            dnl# Mac it does NOT work; we use 'expr STRING : REGEXP'
            dnl# instead.
...
dnl# 5) WX_PORT
dnl# from the previously selected wxWidgets build; this macro must be
dnl# called *after* calling the WX_CONFIG_CHECK macro.
dnl#
dnl# Note that the WX_VERSION_MAJOR, WX_VERSION_MINOR symbols are set
dnl# by WX_CONFIG_CHECK macro
dnl#-----------------------------------------------------------------------
AC_DEFUN([WX_DETECT_STANDARD_OPTION_VALUES],
        [
        dnl# IMPORTANT: WX_VERSION contains all three major.minor.micro
        dnl#            digits, while WX_RELEASE only the major.minor ones.
        WX_RELEASE="$WX_VERSION_MAJOR""$WX_VERSION_MINOR"
        if test $WX_RELEASE -lt 26 ; then

            AC_MSG_ERROR([
    Cannot detect the wxWidgets configuration for the selected wxWidgets
    build since its version is $WX_VERSION < 2.6.0; please install a newer
    version of wxWidgets.
        ...
        if test "$TOOLKIT" = "auto" ; then
            ...
            dnl# NOTE: backward-compatible check for wx2.8; in wx2.9 the
            dnl#       Mac ports are called 'osx_cocoa' and
            dnl#       'osx_carbon' (see above)
        ...
        dnl# Avoid problem described in the WX_STANDARD_OPTIONS which
        dnl# happens when the user gives the options:
        dnl#      ./configure --enable-shared --without-wxshared
        dnl# or just do
    ...
    as static library or '--with-wxshared' to use wxWidgets as shared
    library.
                         ])
        fi
        ...
        dnl#       do NOT need to set them, too
        if test "$DEBUG" = "1"; then
...
dnl# This macro exists just to make configure.ac scripts more readable.
dnl#
dnl# NOTE: you need to use the [" my message"] syntax for 2nd and 3rd
dnl#       arguments if you want that m4 avoid to throw away the spaces
dnl#       prefixed to the argument value, or if that is just your style.
dnl#-----------------------------------------------------------------------
...
dnl#
dnl# Like the WX_STANDARD_OPTIONS_SUMMARY_MSG macro, but these two macros
dnl# also give info about the configuration of the package which used the
dnl# wxpresets.
dnl#
dnl# Typical usage:
dnl#    WX_STANDARD_OPTIONS_SUMMARY_MSG_BEGIN
