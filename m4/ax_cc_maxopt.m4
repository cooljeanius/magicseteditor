...
#   variable. The user can also specify --enable-portable-binary to
...
if test "$ac_test_CFLAGS" != "set"; then
  ...
  case $ax_cv_c_compiler_vendor in
    ...
    ibm) if test "x$acx_maxopt_portable" = xno; then
         ...
         AX_CHECK_COMPILE_FLAG([$xlc_opt],
               ...
               [CFLAGS="-O3 -qansialias -w"
                ...
                echo "*                   ^^^        ^^^                 *"
                echo "* where xxx is pwr2, pwr3, 604, or the  kind of    *"
	...
	[CFLAGS="$CFLAGS -fstrict-aliasing"])
     ...
     ;;
    clang)
     # Apple likes these optimization flags
     CFLAGS="-Os -pipe"
     ;;
  esac
  ...
  if test -z "$CFLAGS"; then
	echo ""
	echo "********************************************************"
        echo "* WARNING: Do not know the best CFLAGS for this system *"
  ...
  AX_CHECK_COMPILE_FLAG([$CFLAGS],[],[
	echo ""
        echo "********************************************************"
        echo "* WARNING: The guessed CFLAGS do not seem to work with *"
        echo "* your compiler.                                       *"
        echo "* Use ./configure CFLAGS=... to specify your own flags *"
        echo "********************************************************"
        echo ""
